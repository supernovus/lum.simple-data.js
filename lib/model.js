...
{
  rules[key] = {key, get: true, set: true};
  return rules;
}, {});
 ...
 * 
 * A really simple example: `{"name": {key: "name", get: true, set: true}}`
 * 
    ...
    for (const dkey in prules)
    {
      ...
      if (!(rule.key in this))
      {
        ...
        if (typeof rule.get === F)
        {
          getter = function() 
          { 
            return rule.get(this.data[dkey], dkey, this);
          }
        }
        else if (rule.get)
        {
          getter = function() { return this.data[dkey] }
        }
        ...
        {
          setter = function(val) { this.data[dkey] = val; }
        }
 ...
 * 
 * If this is `false`, no getter will be assigned at all. This is not
 * recommended unless `set` is also `false`, in which case no accessor
 * property will be assigned at all.
 * 
